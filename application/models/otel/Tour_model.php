...
$query =$this->db->query("select * from acente_tur Where kat_id='$kat_id' and durum=1  and tur_donemi='otel'");
        if( $query->num_rows() > 0 )
        {
            return $query->result_array();
        }
...
$query =$this->db->query("select * from acente_category Where id=$kat_id and durum=1 ");
        if( $query->num_rows() > 0 )
        {
           foreach ($query->result_array() as $row)
										{
...
$query =$this->db->query("select * from acente_tur Where id=$tur_id and durum=1  and tur_donemi='otel'");
		 	 	 ...
		 	 	 function tur_kontrol($tur_id)
     {
        ...
        else
        {
            return FALSE;
			 	 	 ...
			 	 	 function tur_detay_kontrol($tur_kod,$tur_detay_id,$yetiskin_tek_fiyat,$cocuk_tek_fiyat)
     {
	
		$query =$this->db->query("select * from acente_tur_detay Where id=".$tur_detay_id." and yetiskin_fiyat=".$yetiskin_tek_fiyat." and cocuk_fiyat=".$cocuk_tek_fiyat." and tur_kod=".$tur_kod);
        if( $query->num_rows() > 0 )
        {
            return TRUE;
        }
	...
	$query =$this->db->query("select * from acente_tur_detay Where id=".$tur_detay_id." and baslangic_saati='$bas_s' and bitis_saati='$bit_s'");
        ...
        else
        {
            return FALSE;
	 	  	 ...
	 	  	 function sepet_ekle($kul_id,$tur_id,$tur_kod,$tur_detay_id,$yetiskin_adet,$cocuk_adet,$genel_total,$bas_t,$bit_t,$bas_s,$bit_s,$indirimsiz)
		 {
        ...
        if( $query->num_rows() > 0 )
        {
	       ...
	       foreach ($query->result_array() as $row)
	           	...
	           	$query3 =$this->db->query("delete from acente_sepet_detay Where id=".$sepet_id);
	...
	$adet=$yetiskin_adet+$cocuk_adet;
		...
		return TRUE;
		 ...
		 function rez_ekle($kul_id,$tur_id,$tur_kod,$tur_detay_id,$yetiskin_adet,$cocuk_adet,$genel_total,$bas_t,$bit_t,$bas_s,$bit_s,$indirimsiz)
		 {
