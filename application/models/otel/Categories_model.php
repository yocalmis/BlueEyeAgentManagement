...
class Categories_model extends CI_Model
{
	 function __construct()
     {
         ...
         $this->load->database();
	 	 	 ...
	 	 	 function varmi_ana_cat_name($ana_cat_name)
     {
	...
	$query =$this->db->query("select * from tkn_mat_options Where id=1");
        ...
        else
        {
            return FALSE;
	 	 	 ...
	 	 	 function ana_kat_bilgi($ana_cat_name)
     {
	...
	$query =$this->db->query("select * from acente_one_category Where seo_adi='$ana_cat_name' ");
        ...
        else
        {
            return FALSE;
	 	 	 ...
	 	 	 function ana_kat_turlar($ana_cat_name,$uz)
    {
		// return $ana_cat_name;
...
foreach ($query->result_array() as $row)
{
		...
		$n=0; 
		$query_2 = $this->db->query("select * from acente_category Where durum=1  and kategori_turu='otel' and ust_kategori_id=".$row['id']."");	
		$say_alt_cat=$query_2->num_rows($query_2);
			...
			foreach ($query_2->result_array() as $row_2)
				...
				$query_3 = $this->db->query("select * from acente_tur Where durum=1   and tur_donemi='otel'  and kat_id=".$row_2['id']);
				...
				if($say_tur!=0){
					...
					foreach ($query_3->result_array() as $row_3)
						...
						if($say_tur_fiyat==0){ $ana_kat[$i]["tur"][$s]["baslangic_fiyat"]=0;	}
						else{
...
$query =$this->db->query("select * from acente_abone_eposta Where eposta='$email'");
	...
	if($into){return 1;}else{return 0;} 
	 	 	 	 ...
	 	 	 	 function ana_kat_yorumlar($ana_cat_name)
       {
		// return $ana_cat_name;
...
foreach ($query->result_array() as $row)
{
	 ...
	 $i=$i+1;		
		 ...
		 return $ana_kat;
	 	 	 	 ...
	 	 	 	 function ana_kat_populer_kategori($ana_cat_name)
     {
        ...
        if( $query->num_rows() > 0 )
        {
			...
			$query2 =$this->db->query("select * from acente_category Where durum=1  and kategori_turu='otel' and ust_kategori_id='$ids'");
        ...
        else
        {
            return FALSE;
	 	 	 	 ...
	 	 	 	 function ana_kat_populer_kategori_sayi($id)
     {
		 
		
	
		$query =$this->db->query("select * from acente_tur Where durum=1 and tur_donemi='otel' and kat_id=$id");
	 	 	 ...
	 	 	 function populer_tur()
     {
		...
		$query =$this->db->query("select * from acente_tur Where durum=1  and tur_donemi='otel' and cok_satan=1 order by id desc limit 0 , 24");
        if( $query->num_rows() > 0 )
        {
            return $query->result_array();
        }
	 	 	 	 	 ...
	 	 	 	 	 function populer_ana_kategori()
     {
		...
		$query =$this->db->query("select * from acente_one_category Where durum=1 and one_cikan_otel=1 order by id desc limit 0 , 100");
        if( $query->num_rows() > 0 )
        {
            return $query->result_array();
        }
	...
	$query =$this->db->query("select * from acente_puan_servis Where tur_kod='$kod'");
        ...
        else
        {
            return FALSE;
			  ...
			  function yorum_puan_ortalama($kod,$kullanici_id)
     {
