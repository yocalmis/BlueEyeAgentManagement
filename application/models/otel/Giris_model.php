...
function user_return($kul,$sf)
     {

	$query =$this->db->query("select * from acente_uyeler Where username='$kul' and pass='$sf' and status=1");
	 $query->num_rows();
        if( $query->num_rows() > 0 )
        {
	...
	$query =$this->db->query("select * from tkn_mat_options Where id=1");
	 ...
	 function turlar_home($uz)
     {
		  $i=0;
		 $query = $this->db->query('select * from acente_one_category Where one_cikan_otel=1 and durum=1 order by id asc limit 0 , 3');
...
foreach ($query->result_array() as $row)
{
		...
		$n=0; 
		$query_2 = $this->db->query("select * from acente_category Where durum=1 and kategori_turu='otel'  and ust_kategori_id=".$row['id']." order by id asc limit 0 , 4");	
		$say_alt_cat=$query_2->num_rows($query_2);
			...
			foreach ($query_2->result_array() as $row_2)
				...
				$query_3 = $this->db->query("select * from acente_tur Where durum=1  and kat_id=".$row_2['id']."  and tur_donemi='otel'");
	...
	if($into){return 1;}else{return 0;} 
	 	 	 	 	 ...
	 	 	 	 	 function populer_ana_kategori()
     {
        ...
        else
        {
            return FALSE;
	 	 	 	 	 ...
	 	 	 	 	 function populer_ana_kategori_sayi($id)
     {
        ...
        if( $query->num_rows() > 0 )
        {
			$total=0;
                foreach ($query->result_array() as $row)
						...
						$query2 = $this->db->query("select * from acente_tur Where durum=1  and tur_donemi='otel' and kat_id=".$row['id']);							
	 	 	 ...
	 	 	 function populer_kategori()
     {
        ...
        else
        {
 	 	 ...
 	 	 function uye_kaydet($isim,$kl,$sf,$em)
     {
