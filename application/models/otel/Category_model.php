...
class Category_model extends CI_Model
{
	 function __construct()
     {
         ...
         $this->load->database();
	 	 	 ...
	 	 	 function varmi_cat_name($cat_name)
     {
	
		$query =$this->db->query("select * from acente_category Where seo_adi='$cat_name' and kategori_turu='otel'");
        if( $query->num_rows() > 0 )
        {
            return TRUE;
        }
	...
	$query =$this->db->query("select * from tkn_mat_options Where id=1");
        ...
        else
        {
            return FALSE;
	 	 	 ...
	 	 	 function alt_kat_bilgi($cat_name)
     {
...
$query =$this->db->query("select * from acente_category Where seo_adi='$cat_name'  and kategori_turu='otel'");
        ...
        else
        {
            return FALSE;
	 	 	 ...
	 	 	 function alt_katen_populer($cat_name)
     {
	 	 	 ...
	 	 	 function alt_kat_turlar($cat_name)
     {
	  ...
	  function alt_kat_turlar_fiyat($tur_kod)
     {
...
$query =$this->db->query("select * from acente_tur_detay Where tur_kod='$tur_kod' and durum=1  order by yetiskin_fiyat asc limit 0 , 1");
        ...
        else
        {
          return 0;
		 	 	 ...
		 	 	 function alt_kat_turlar_yorum_sayi($tur_kod)
     {
	...
	$query =$this->db->query("select * from acente_yorumlar Where tur_kod='$tur_kod' and durum=1");
     return $query->num_rows();
		 	 	 ...
		 	 	 function ust_kat_bilgi_getir($ust_kategori_id,$uz)
     {
	...
	$query =$this->db->query("select * from acente_one_category Where id='$ust_kategori_id'");
   	foreach ($query->result_array() as $row)
	 	 	 ...
	 	 	 function ana_kat_kategorileri($id)
     {
	
$query =$this->db->query("select * from acente_category Where ust_kategori_id=".$id." and kategori_turu='otel'");
        if( $query->num_rows() > 0 )
        {
            return $query->result_array();
        }
        else
        {
            return FALSE;
	 	 	 	 ...
	 	 	 	 function ana_kat_kategorileri_tur_adet($id)
     {
		 
	
	$query =$this->db->query("select * from acente_tur Where kat_id=".$id." and durum=1  and tur_donemi='otel'");
     return $query->num_rows();
	 } 
	 
	 
	 	 
	 	 	 function kat_yorumlar($id)
     {
	$n=0;
	$yorum="";
	...
	foreach ($query4->result_array() as $row4)
										{
											...
											$yorum[$n]["kullanici"]=$row4['username']; 
