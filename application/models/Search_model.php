	...
	$query =$this->db->query("select * from acente_tur_detay Where tur_kod=".$kod." order by yetiskin_fiyat asc limit 0 , 1");       
        ...
        else
        {
            return FALSE;
	 	  	 ...
	 	  	 function fyt_ind_onc($kod)
     {

	
	$query =$this->db->query("select * from acente_tur_detay Where tur_kod=".$kod." order by yetiskin_fiyat_ind_once asc limit 0 , 1");       
        if( $query->num_rows() > 0 )
        {
            
										foreach ($query->result_array() as $row)
										{
											return $row['yetiskin_fiyat_ind_once'];
	...
	$query =$this->db->query("select * from acente_puan_servis Where tur_kod='$kod'");
        ...
        else
        {
            return FALSE;
	 	 	 ...
	 	 	 function tur_tarih_uygun($kod,$bas_t,$bit_t)
     {
