...
class Ajax_process extends CI_Controller
{
    ...
    public function comment_ajax()
    {
        ...
        $baslik = trim($baslik);
        $baslik = htmlentities($baslik);
        $baslik = strip_tags($baslik);
        $yorum = trim($yorum);
        $yorum = htmlentities($yorum);
        $yorum = strip_tags($yorum);
        $kul_id = trim($kul_id);
        $kul_id = htmlentities($kul_id);
        $kul_id = strip_tags($kul_id);
        $tur_id = trim($tur_id);
        $tur_id = htmlentities($tur_id);
        $tur_id = strip_tags($tur_id);
        $tur_kod = trim($tur_kod);
        $tur_kod = htmlentities($tur_kod);
        $tur_kod = strip_tags($tur_kod);
        $servis = trim($servis);
        $servis = htmlentities($servis);
        $servis = strip_tags($servis);
        $organ = trim($organ);
        $organ = htmlentities($organ);
        $organ = strip_tags($organ);
        $para = trim($para);
        $para = htmlentities($para);
        $para = strip_tags($para);
        $guven = trim($guven);
        $guven = htmlentities($guven);
        $guven = strip_tags($guven);
...
/**/
    ...
    public function yeni_kart()
    {
        ...
        $array["user_id"] = trim($array["user_id"]);
        $array["user_id"] = strip_tags($array["user_id"]);
        $array["user_id"] = htmlentities($array["user_id"]);

        $array["cardNumber"] = trim($array["cardNumber"]);
        $array["cardNumber"] = strip_tags($array["cardNumber"]);
        $array["cardNumber"] = htmlentities($array["cardNumber"]);

        $array["cardName"] = trim($array["cardName"]);
        $array["cardName"] = strip_tags($array["cardName"]);
        $array["cardName"] = htmlentities($array["cardName"]);

        $array["cardExpDate"] = trim($array["cardExpDate"]);
        $array["cardExpDate"] = strip_tags($array["cardExpDate"]);
        $array["cardExpDate"] = htmlentities($array["cardExpDate"]);

        $array["cardCvc"] = trim($array["cardCvc"]);
        $array["cardCvc"] = strip_tags($array["cardCvc"]);
        $array["cardCvc"] = htmlentities($array["cardCvc"]);
