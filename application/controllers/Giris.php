...
class Giris extends CI_Controller {
	...
	/*********Cache Open***********/
	if($this->session->userdata('useronline')==""){ $cachedosyasi = "cache/ip_".md5($_SERVER['REMOTE_ADDR'])."_tur_anasayfa.php";	}
	else{ $cachedosyasi = "cache/kull_".$this->session->userdata('kul_id')."_tur_anasayfa.php"; }
	...
	/*********Cache close***********/
		$ch = fopen($cachedosyasi, 'w'); fwrite($ch, ob_get_contents()); fclose($ch); ob_end_flush();
	/*********Cache close***********/	
	...
	if($this->session->userdata('lng_turu')=="Esp"){  $uz="_esp"; $data["uz"]="_esp"; } 
		...
		$sf=$this->input->post('ps',TRUE);					
		 $kul=trim($kul); 		 $kul=htmlentities($kul);      $kul=strip_tags($kul);  //  $kul=strtolower($kul);	
		$sf=trim($sf); $sf=htmlentities($sf); $sf=strip_tags($sf);			
		$sf=md5($sf);		
