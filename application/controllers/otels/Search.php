...
class Search extends CI_Controller {
	 function __construct()
     {
		...
		echo $this->funcs->index($this->session->userdata('currency'),$this->session->userdata('lng'));	    
	...
	if($this->session->userdata('lng_turu')=="Ru"){  $uz="_ru"; $data["uz"]="_ru"; } 	
		...
		$this->load->model('otel/Search_model');		
...
/*
	...
	*/		 
		...
		$data["kat_getir"][$n]=$this->Search_model->kat_getir($dizi["kat_id"]);
		...
		$data["tur_puan_genel"][$n]=$data["tur_puan_genel"][$n]/4;		
		 ...
		 if(($dizi["tur_donemi"]=="otel")and($data["tur_fyt_uygun"][$n]==1)){ $data["arama_sayi"]=$data["arama_sayi"]+1;} 
		...
		$this->load->view('otel/search',$data);
